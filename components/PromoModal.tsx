...
const PromoModal: React.FC<PromoModalProps> = ({ isOpen, onClose, adminPhoneNumber }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-80 flex items-center justify-center z-[100] p-4 transition-opacity duration-300 ease-in-out">
      <div className="bg-gradient-to-br from-gray-800 via-gray-900 to-black p-5 sm:p-6 md:p-8 rounded-xl shadow-2xl w-full max-w-lg space-y-4 sm:space-y-6 text-center border-2 border-yellow-500 transform transition-all duration-300 ease-in-out scale-95 opacity-0 animate-fadeInScaleUp">
        <button
          onClick={onClose}
        ...
        <div className="relative w-full h-48 sm:h-56 md:h-64 overflow-hidden rounded-lg shadow-md mb-3 sm:mb-4 bg-gray-700/30">
        ...
        <h3 className="text-xl sm:text-2xl font-bold text-yellow-400 mt-2">Penawaran Spesial Untuk Anda!</h3>
        <p className="text-sm sm:text-base text-gray-300">
          Nikmati bonus kredit berlimpah dengan penawaran spesial kami. Jangan lewatkan kesempatan ini!
        </p>

        <div className="space-y-2 text-left px-2 sm:px-4">
          {creditTiers.map((tier, index) => (
            <div key={index} className="p-2 sm:p-3 bg-gray-700/50 rounded-md flex justify-between items-center text-xs sm:text-sm">
        ...
        <div className="mt-4 mb-2 text-sm text-gray-200">
        ...
        <div className="flex flex-col sm:flex-row justify-center gap-3 sm:gap-4 pt-2 sm:pt-3">
          <a
            href={`https://wa.me/${adminPhoneNumber}?text=${whatsappMessage}`}
            target="_blank"
            rel="noopener noreferrer"
            className="w-full sm:w-auto bg-green-500 hover:bg-green-600 text-white font-semibold py-2.5 px-5 sm:px-6 rounded-lg transition-colors flex items-center justify-center gap-2 text-sm sm:text-base shadow-lg hover:shadow-xl transform hover:scale-105"
          >
          ...
          <button
            onClick={onClose}
            className="w-full sm:w-auto bg-gray-600 hover:bg-gray-500 text-white font-semibold py-2.5 px-5 sm:px-6 rounded-lg transition-colors text-sm sm:text-base shadow-lg hover:shadow-xl"
          >
            Tutup
          </button>
        </div>
